...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sub = max_length - length_sentence
        if sub > 0:
            # sentence.append(sub * zero_vector)
            sentence += [zero_vector] * sub
            labels = labels + (sub * [zero_label])  #labels += sub * [zero_label] is incorrect!  see http://justjavac.iteye.com/blog/1827927
            ret.append((sentence, labels, [True] * length_sentence + [False] * sub))
        elif sub <= 0:
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable(name='U', shape=(Config.hidden_size, Config.n_classes), initializer=tf.contrib.layers.xavier_initializer(),dtype=tf.float32)
        b2 = tf.get_variable(name='b2', shape=(Config.n_classes, ), initializer=tf.constant_initializer(value=0), dtype=tf.float32)
        # h_0 = tf.get_variable(name='h_0',shape=((tf.shape(x))[0], Config.hidden_size), initializer=tf.constant_initializer(value=0, dtype=tf.float32))
        input_shape = tf.shape(x)
        h_0 = tf.zeros( (input_shape[0], Config.hidden_size) )
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h_0 = cell(x[:,time_step,:], h_0, scope='RNN')
