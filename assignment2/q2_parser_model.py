...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_placeholder = tf.placeholder(shape=(None, Config.n_features), dtype=tf.int32)
        self.labels_placeholder = tf.placeholder(shape=(None, Config.n_classes), dtype=tf.float32)
        self.drop_placehodler = tf.placeholder(dtype=tf.float32)
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, shape=(-1, Config.n_features * Config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = initializer((Config.n_features * Config.embed_size, Config.hidden_size))
        U = initializer((Config.hidden_size, Config.n_classes))
        b1 = tf.Variable(np.zeros(Config.hidden_size, dtype=np.float32))
        b2 = tf.Variable(np.zeros(Config.n_classes, dtype=np.float32))
        h = tf.nn.relu(tf.matmul(x, W) + b1)
        h_drop = tf.nn.dropout(h, keep_prob=self.drop_placehodler)
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        softmax_result = tf.nn.softmax_cross_entropy_with_logits(pred, self.labels_placeholder)
        loss = tf.reduce_mean(softmax_result)
        ### END YOUR CODE
        return loss
