...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x_square = x * x
    y = np.sqrt(np.sum(x_square, axis=1, keepdims=True))
    y.reshape(1, len(y))
    x = np.divide(x, y)
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    # grad = p.copy()
    # grad[target] = (grad[target] - 1).dot(predicted.T)
    zeros = np.zeros(y_hat.shape)
    zeros[target] = 1
    y = zeros
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    grad = np.zeros(outputVectors.shape)
    grad[target] = predicted * (sigmoid(outputVectors[target].dot(predicted)) - 1)
