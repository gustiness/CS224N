...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for center_index in range(len(sentence)):
            window = []
            for window_word_index in range(center_index - window_size, center_index + window_size + 1):
                if window_word_index < 0:
                    window += start
                elif window_word_index > len(sentence) - 1:
                    window += end
                else:
                    window += sentence[window_word_index]
            windowed_data.append((window, labels[center_index]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(dtype=tf.int32, shape=(None, Config.n_window_features))
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, shape=(-1, Config.n_window_features * Config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_loss_op(self, pred):
        ...
        softmax_result = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred, labels=self.labels_placeholder)
